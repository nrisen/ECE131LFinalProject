...
int main() {
	...
	while(menuChoice != 6) {
		...
		if(menuChoice == 1) {
			...
			//Sets all letters to unguessed for the alphabet tracker
			for(int i = 0; i < 26; i++) {
				activeGame.letterStatus[i] = 0; 
			}
...
void play(struct MyGame *game) {
	...
	while((*game).turnsTaken < 6 && winFlag == 0) {
        ...
        // Print loop and alphabet tracker update
        for(int i = 0; i < 5; i++) {
			...
			if(result[i] == 2) {
				...
				if((*game).letterStatus[letterIndex] != 1) { // Only update if not already marked correct
					(*game).letterStatus[letterIndex] = 3;
				}
			} else {
				printf("\033[1;31m[WRONG]\033[0m ");
				if((*game).letterStatus[letterIndex] != 1 && (*game).letterStatus[letterIndex] != 3) { // Only update if not already marked correct
...
void printAlphabet(struct MyGame *game) {
	...
	for(int i = 0; i < 26; i++) {
		...
		else if((*game).letterStatus[i] == 3) {
			printf("\033[1;33m%c\033[0m ",('A' + i));
		}
		else {
